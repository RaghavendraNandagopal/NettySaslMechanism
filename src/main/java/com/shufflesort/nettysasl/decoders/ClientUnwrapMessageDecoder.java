...
public class ClientUnwrapMessageDecoder extends FrameDecoder {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ClientUnwrapMessageDecoder.class);
	...
	protected Object decode(final ChannelHandlerContext ctx, final Channel ch,
		...
		if (isUnwrap) {
			...
			if (available >= 4) {
				...
				logger.debug("Client Unwrapped the message successfully");
			}
		}

		logger.debug("Client MessageDecoder called");
	...
	private boolean isUseUnwrap(final SaslNettyClient saslNettyClient) {
		...
		if (saslNettyClient == null) {
			logger.debug("Unwrapping Client Message decoder called, saslNettyClient is null hence returning the same object");
			return false;
		}

		// If sasl authentication is not completed yet.
		if (saslNettyClient != null && !saslNettyClient.isComplete()) {
			logger.debug("Unwrapping Client Message decoder called, saslNettyClient is not authenticated at first place, hence returning the same object");
			return false;
		}
		...
		if (saslNettyClient != null && !saslNettyClient.isUseWrap()) {
			logger.debug("Unwrapping Client Message decoder called, saslNettyClient is not null but useWrap is false hence returning the same object");
			return false;
		}

		logger.debug("Unwrapping Client pay load message ");
		return true;
	}
