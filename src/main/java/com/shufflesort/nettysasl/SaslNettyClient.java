...
public class SaslNettyClient {
	...
	private static class SaslClientCallbackHandler implements CallbackHandler {
		...
		public void handle(final Callback[] callbacks)
			...
			if (nc != null) {
				logger.debug(
						"handle: SASL client callback: setting username: ",
						userName);
				nc.setName(userName);
			}
			if (pc != null) {
				logger.debug("handle: SASL client callback: setting userPassword");
				pc.setPassword(userPassword);
			}
			if (rc != null) {
				logger.debug("handle: SASL client callback: setting realm: ",
						rc.getDefaultText());
	...
	public SaslNettyClient(final String userName, final byte[] token) {
		try {
			logger.debug("SaslNettyClient: Creating SASL "
			...
			logger.debug("SaslNettyClient: Could not obtain topology token for Netty "
	...
	public byte[] saslResponse(final SaslMessageToken saslTokenMessage) {
		try {
			...
			logger.error("saslResponse: Failed to respond to SASL server's token:"
					+ e.getMessage());
			return null;
		}
	}
	...
	public void setUseWrap() {
		...
		logger.debug("saslClient Negotiated quality of service: " + qop);
		useWrap = qop != null && !"auth".equalsIgnoreCase(qop);
		logger.debug("Setting SaslNettyClient useWrap to " + useWrap);
	}
